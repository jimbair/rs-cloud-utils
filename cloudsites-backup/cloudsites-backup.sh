...
# Where is the PATH set?  You don't want to execute any trojan horses.
PATH=/bin
...
today=$(date +%A)
if [ -z "${today}" ]; then
    echo "Unable to find today's date." >&2
    exit 1
fi
...
for i in ${storage}; do
    ...
    ourPath="$(readlinke -f /mnt/${i}/${custNum})"
    ...
    cfBackup="$(readling -f ${ourPath}/bin/cloudsites-push.py)"
    if [ ! -s "${cfBackup}" ]; then
    ...
    dbBackup="$(readlink -f ${ourPath}/bin/cloudsites-mysql.py)"
    if [ ! -s "${dbBackup}" ]; then
    ...
    cfRotate="$(readlink -f ${ourPath}/bin/cloudsites-rotate.py)"
    if [ ! -s "${dbBackup}" ]; then
